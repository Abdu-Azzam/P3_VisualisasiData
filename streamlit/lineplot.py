import streamlit as st
import matplotlib.pyplot as plt

# Sample data
months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
product_A_sales = [10, 20, 15, 25, 30, 45, 50, 60, 55, 65, 70, 80]
product_B_sales = [8, 12, 18, 22, 28, 35, 40, 45, 50, 55, 60, 65]

# Streamlit layout
st.title("Visualisasi Penjualan Produk")
st.sidebar.header("Pengaturan Produk")
option = st.sidebar.selectbox(
    "Pilih Tipe Visualisasi",
    ("Line Plot", "Customisasi Line Plot", "Trend Line Plot", "Subplot")
)

def line_plot():
    fig, ax = plt.subplots()
    ax.plot(months, product_A_sales, label='Product A', color='blue', marker='o')
    ax.set_title('Penjualan Produk A Per Bulan')
    ax.set_ylabel('Jumlah Penjualan')
    ax.set_xlabel('Bulan')
    ax.set_xticks(months)
    ax.legend()
    ax.grid(True)
    st.pyplot(fig)

def customize_line_plot():
    fig, ax = plt.subplots()
    ax.plot(months, product_A_sales, label='Product A', color='blue', linestyle='-', marker='o')
    ax.plot(months, product_B_sales, label='Product B', color='red', linestyle='-', marker='x')
    ax.set_title('Penjualan Produk A & B Per Bulan')
    ax.set_ylabel('Jumlah Penjualan')
    ax.set_xlabel('Bulan')
    ax.set_xticks(months)
    ax.legend()
    ax.grid(True)
    st.pyplot(fig)

def trend_lines_plot():
    fig, ax = plt.subplots()
    ax.plot(months, product_A_sales, label='Product A Trend', linestyle='--', color='blue')
    ax.plot(months, product_B_sales, label='Product B Trend', linestyle='--', color='red')
    ax.set_title('Tren Penjualan Produk A & B Per Bulan')
    ax.set_ylabel('Jumlah Penjualan')
    ax.set_xlabel('Bulan')
    ax.set_xticks(months)
    ax.legend()
    ax.grid(True)
    st.pyplot(fig)

def subplots():
    fig, ax = plt.subplots(2, 1, figsize=(10, 8))

    ax[0].plot(months, product_A_sales, label='Product A', color='blue', marker='o')
    ax[0].set_title('Penjualan Produk A Per Bulan')
    ax[0].set_ylabel('Jumlah Penjualan')
    ax[0].set_xlabel('Bulan')
    ax[0].legend()
    ax[0].grid(True)

    ax[1].plot(months, product_B_sales, label='Product B', color='red', marker='x')
    ax[1].set_title('Penjualan Produk B Per Bulan')
    ax[1].set_ylabel('Jumlah Penjualan')
    ax[1].set_xlabel('Bulan')
    ax[1].legend()
    ax[1].grid(True)

    plt.tight_layout()
    st.pyplot(fig)

# Eksekusi
if option == "Line Plot":
    line_plot()
elif option == "Customisasi Line Plot":
    customize_line_plot()
elif option == "Trend Line Plot":
    trend_lines_plot()
elif option == "Subplot":
    subplots()
